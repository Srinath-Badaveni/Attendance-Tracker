...
async function runSync() {
  ...
  try {
    ...
    const response = await page.goto("http://103.171.190.44/TKRCET/");
    ...
    const attendanceData = await page.evaluate(() => {
      ...
      const allTables = Array.from(
        document.querySelectorAll("table.darker-border-table"),
      );

      // Find the report table (Overall Stats)
      const reportTable = allTables.find((t) =>
        t.innerText.includes("Subject Abrevation"),
      );
      ...
      const daywiseTable = allTables.find((t) => {
        ...
        return (
          t.innerText.includes("Daywise Detailed Attendance") ||
          (prevElement &&
            prevElement.innerText.includes("Daywise Detailed Attendance"))
        );
      });

      // 2. Extract Student Name
      const nameEl = Array.from(document.querySelectorAll("p")).find((p) =>
        p.innerText.includes("Name:"),
      );
      const studentName = nameEl
        ? nameEl.innerText.split("Name:")[1]?.trim()
        : "Student";
      ...
      const statsRow = Array.from(
        reportTable.querySelectorAll("tbody tr"),
      ).find((r) => r.innerText.includes("Attendance"));
    ...
    const periodsHtml = latestDay.periods
      .map((p, i) => `<li>Period ${i + 1}: <b>${p}</b></li>`)
      .join("");
